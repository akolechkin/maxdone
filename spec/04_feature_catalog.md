...
- 🔲 **Каталог шаблонов**: опубликованные/черновики (`findPublishedGoalTemplates /
  findDraftGoalTemplates`), счётчики использований (`getPublishedGoalTemplateGoalCounts`).
- 🔲 **Создание цели из шаблона**: `Manager.createGoalFromTemplate`,
  `CreateGoalFromTemplateAsyncTask`. Выбор даты старта → относительные сроки
  (`*Days`) разворачиваются в реальные даты.
- 🔲 **Майлстоуны и Key Results**: KeyResultType SIMPLE/SUM_RESULT/LAST_RESULT,
  плановое значение `planned`. Группировка задач по майлстоунам
  (`groupTaskTemplatesByMilestones`).
- 🔲 **Задачи из шаблона защищены**: `BLOCKED_BY_GOAL_TEMPLATE` — нельзя свободно
  редактировать задачу, пришедшую из шаблона.
