...
- ⊘ **Задачи из шаблона «защищены»**: BR-T1 ОТМЕНИЛ блокировку (`BLOCKED_BY_GOAL_TEMPLATE`).
  В single-user задача из своего шаблона редактируема; `Task.from_template` остался лишь
  пассивным маркером происхождения (плашка в редакторе), без 403/read-only.
