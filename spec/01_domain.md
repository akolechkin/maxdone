...
## Примечание Milestone 2 (см. spec/06_milestone2.md)
- `task_type` — НЕ источник истины горизонта. Горизонт выводится из `due_date`
  на лету (spec/06, BR-19/BR-20); поле сохраняется денормализованно в `Task.save()`
  для совместимости/импорта, но списки и счётчики по нему не фильтруются.
- Форма задачи не показывает выбор горизонта; новая задача без `due_date` → INBOX (BR-21).
